...
try:
    with io.open(filepath, "r", encoding="utf-8") as file:
        content = file.read()
        content = content.strip().replace(u"\u202a", "").replace(u"\u200e", "").replace(u"\u202c", "")
except IOError as e:
...
# searchString="""
# [5/22/18, 00:57:07] Afraz: <200e>video omitted^M
# [5/22/18, 00:57:09] Afraz: <200e>image omitted^M
...
for match in pattern.finditer(content):
        #Use this if US Easter Time Zone
...
df_pd = df_pd[(df_pd['date'] > focus_month)]
...
df_pd['fwd_messages'] = (df_pd['message'].str.contains("omitted").fillna(0).astype(int))
df_pd['actual_messages'] = ((~df_pd['message'].str.contains("omitted")).fillna(0).astype(int))


logging.debug(df_pd)

#df_pd = df_pd[~df_pd['message'].str.contains("omitted")]
...
df_group_by_conv['duration'] = (df_group_by_conv['date_max'] - df_group_by_conv['date_min']).apply(lambda x: int(x.total_seconds() / 60))
...
group_by_user = df_pd.groupby(df_pd['user']).sum()[['actual_messages','fwd_messages']].sort_values('actual_messages',ascending=False).head(15)
...
if not group_by_user.empty:
    ...
    for i, (v_fwd,v_act) in enumerate(zip(group_by_user["fwd_messages"],group_by_user["actual_messages"])):
        ...
        group_by_user_plot.text(i, v_act-15, v_act, rotation=90)
...
text = " ".join(m for m in df_pd['message'].apply(get_words))

logging.debug("There are {} words in the combination of all messages.".format(len(text)))
